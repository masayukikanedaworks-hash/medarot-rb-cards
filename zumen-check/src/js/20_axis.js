...
(function (ZC) {
  ...
  const PARAMS = {
    ...
    MIN_LEN_BUBBLE: 20,    // 符号バブルがある位置は部分的な短い芯でも採用する最小長さ
    FRAME_EDGE_TOL: 3,     // 内容範囲の端からこの距離以内なら図枠の疑い
    LABEL_LATERAL: 30,     // 芯の位置とバブル中心の横ずれの許容
    BUBBLE_LATERAL: 3,     // 短い芯をバブルで救うときの横ずれの許容（符号は芯の真上にある）
    ...
    GROUP_R_TOL: 0.18,     // 同じ図の符号バブルとみなす半径の相対差
  };
  ...
  function detect(extract) {
    ...
    const allCands = bubbleCandidates(extract.texts, extract.circles || []);
    const grouped = groupBubbles(allCands);
    const cands = grouped.primary;
    ...
    return {
      ...
      h: hAxes,
      bbox,
      // 対象にした図（符号バブルの広がりが最大の図）と、除外した図の情報
      region: grouped.groups.length > 1 ? grouped.groups[0].bbox : null,
      groups: grouped.groups.map((g) => ({ bbox: g.bbox, r: g.r, count: g.items.length })),
      droppedBubbles: allCands.length - cands.length,
    };
  }
  ...
  function buildAxes(pieces, dir, extentRef, edge0, edge1, cands) {
    ...
    for (const c of clusters) {
      ...
      // 符号は芯の真上に描かれるので、救済は「バブル中心が芯の線上」に限る。
      // 緩めると近くの壁線などが符号を横取りしてしまう。
      const hasBubble = (cands || []).some((t) => {
        const lateral = dir === "v" ? Math.abs(t.x - pos0) : Math.abs(t.y - pos0);
        if (lateral > PARAMS.BUBBLE_LATERAL) return false;
  ...
  function bubbleCandidates(texts, circles) {
    ...
    for (const t of texts) {
      ...
      cands.push({ norm: lab.label, dir: lab.dir, x: host.x, y: host.y, r: host.r, used: false });
    }
    return cands;
  }

  // 1枚の用紙に複数の図（右下のキープラン等）が載っている場合、符号バブルの
  // 大きさで図を分け、一番大きく描かれている図だけを拾い出しの対象にする。
  // 図どうしの隙間は図の中の芯間隔より狭いことがあるため、位置ではなく大きさで分ける。
  function groupBubbles(cands) {
    const sized = cands.filter((c) => c.r > 0);
    if (sized.length < 2) return { primary: cands, groups: [] };
    const groups = [];
    let g = null;
    for (const c of sized.slice().sort((a, b) => a.r - b.r)) {
      if (g && c.r <= g.rMax * (1 + PARAMS.GROUP_R_TOL)) {
        g.items.push(c);
        g.rMax = Math.max(g.rMax, c.r);
      } else {
        g = { items: [c], rMin: c.r, rMax: c.r };
        groups.push(g);
      }
    }
    for (const gr of groups) {
      ...
      gr.bbox = { x0, y0, x1, y1 };
      gr.r = (gr.rMin + gr.rMax) / 2;
      // 「大きく描かれている図」= 符号バブルの広がりが最も大きい図
      gr.spread = Math.max(x1 - x0, 0) + Math.max(y1 - y0, 0);
    }
    if (groups.length < 2) return { primary: cands, groups };
    groups.sort((a, b) => b.spread - a.spread || b.items.length - a.items.length);
    return { primary: groups[0].items, groups };
  }
  ...
  ZC.axis = { detect, displayName, bubbleCandidates, groupBubbles, PARAMS, SIDES, SIDE_NAME };
