...
5. **既定でチェックONになるのは「X○○/Y○○の符号が付いた芯」のみ**（図枠疑いは除く）。
   実図面には符号なしの鎖線（設備芯・隠れ線）が大量にあるため、鎖線だけでは
   既定ONにしない。壁線・寸法線・図枠などの誤検出候補は一覧に出しつつ既定OFFにして、
   利用者がチェックボックスで最終判断する（README の仕様）。
6. 実図面では符号が芯の上下（左右）両端に 2 回書かれる。同一符号は方向ごとに
   1 本の芯にのみ割り当て、割り当ては「横ずれの小ささ」を最優先でスコアリングする
   （少し離れた壁線が余った符号を横取りしないように）。
